...
const LastWord_2 = (str) => {
    ...
    return newStr.slice(str.lastIndexOf()).join(" ");
...
const replaceWords = (str) => {
    // write your code here
    let newStr = str.split(" ")
    let x = newStr.indexOf("I")
    let y = newStr.indexOf("am");
    let z = newStr.indexOf("was");
    for (let i=0 ;i<newStr.length;i++){
        if (newStr[i]== "I") {
            newStr.splice(x ,1,"We");
        }
        if (newStr[i]== "am") {
            newStr.splice(y ,1,"are");
        }
        if (newStr[i]== "was") {
            newStr.splice(z ,1,"were");
        }
    }
    return newStr.join(" ");
